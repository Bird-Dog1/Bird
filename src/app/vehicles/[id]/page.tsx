...
export default async function VehicleDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-8 px-4 py-10 sm:px-6 lg:px-8"><div className="grid gap-8 lg:grid-cols-[1.1fr_0.7fr]"><section>{vehicle.vehicle_photos.length > 0 ? <div className="grid gap-4 sm:grid-cols-2">{vehicle.vehicle_photos.map((photo, index) => <div className="relative aspect-[16/10] overflow-hidden rounded-3xl bg-secondary" key={photo.id}>{photo.signed_url ? <Image alt={`${title} photo ${index + 1}`} className="object-cover" fill sizes="(min-width: 1024px) 50vw, 100vw" src={photo.signed_url} /> : <div className="flex h-full items-center justify-center text-sm text-muted-foreground">Photo unavailable</div>}</div>)}</div> : <div className="flex aspect-[16/8] items-center justify-center rounded-3xl bg-secondary text-muted-foreground">Photos coming soon</div>}</section><aside className="space-y-6"><Card><CardHeader><CardTitle>{title}</CardTitle><p className="text-sm text-muted-foreground">{vehicle.city}, {vehicle.state} · {vehicle.dealerships.name}</p></CardHeader><CardContent className="space-y-6"><div className="grid grid-cols-2 gap-4"><Detail label="Monthly price" value={formatCurrency(vehicle.monthly_price)} /><Detail label="Deposit" value={formatCurrency(vehicle.deposit)} /><Detail label="Mileage limit" value={vehicle.mileage_limit ? `${vehicle.mileage_limit}/mo` : "Ask dealer"} /><Detail label="Minimum age" value={`${vehicle.minimum_age}+`} /><Detail label="Insurance" value={vehicle.insurance_required ? "Required" : "Ask dealer"} /><Detail label="Rideshare" value={vehicle.rideshare_allowed ? "Allowed" : "Not listed"} /></div>{vehicle.description ? <p className="text-sm text-muted-foreground">{vehicle.description}</p> : null}<div className="rounded-2xl border border-border bg-background/40 p-4 text-sm text-muted-foreground">Approval is not guaranteed. Valid license and active insurance required. Final approval, contract, and payment are handled by the dealership.</div><Button asChild className="w-full" size="lg"><Link href={`/vehicles/${vehicle.id}/apply` as Route}>Apply for this vehicle</Link></Button></CardContent></Card><Card><CardHeader><CardTitle>Dealership</CardTitle></CardHeader><CardContent className="space-y-2 text-sm text-muted-foreground"><p className="font-semibold text-foreground">{vehicle.dealerships.name}</p><p>{[vehicle.dealerships.address, vehicle.dealerships.city, vehicle.dealerships.state, vehicle.dealerships.zip].filter(Boolean).join(", ")}</p>{vehicle.dealerships.phone ? <p>{vehicle.dealerships.phone}</p> : null}{vehicle.dealerships.website ? <a className="text-primary hover:underline" href={vehicle.dealerships.website}>{vehicle.dealerships.website}</a> : null}</CardContent></Card></aside></div></main>
  );
}
function Detail({ label, value }: { label: string; value: string }) { return <div className="rounded-2xl border border-border bg-background/40 p-4"><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 font-semibold">{value}</p></div>; }
