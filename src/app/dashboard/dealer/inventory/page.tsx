...
export default async function DealerInventoryPage() {
  ...
  return <div className="space-y-6"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="text-xs font-semibold uppercase tracking-[0.32em] text-muted-foreground">Dealer inventory</p><h1 className="mt-3 text-4xl font-semibold tracking-[-0.05em]">Inventory</h1><p className="mt-2 text-muted-foreground">Add, edit, price, and publish dealership vehicles.</p></div><Button asChild><Link href="/dashboard/dealer/inventory/new">Add vehicle</Link></Button></div>{error ? <EmptyState title="Vehicles could not load" description={error.message} /> : vehicles.length === 0 ? <EmptyState action={<Button asChild><Link href="/dashboard/dealer/inventory/new">Add your first vehicle</Link></Button>} description="Vehicles you add will be protected by dealership RLS." title="No inventory yet" /> : <div className="grid gap-4">{vehicles.map((vehicle) => <Card className="transition hover:border-white/20" key={vehicle.id}><CardContent className="flex flex-col justify-between gap-4 p-5 md:flex-row md:items-center"><div><div className="flex flex-wrap items-center gap-3"><h2 className="text-xl font-semibold tracking-[-0.03em]">{vehicleTitle(vehicle)}</h2><StatusBadge value={vehicle.status} /></div><p className="mt-1 text-sm text-muted-foreground">VIN {vehicle.vin} · {vehicle.city}, {vehicle.state} · {vehicle.vehicle_photos.length} photos</p></div><div className="flex flex-col gap-3 sm:flex-row sm:items-center"><p className="font-semibold text-primary">{formatCurrency(vehicle.monthly_price)}/mo</p><Button asChild variant="outline"><Link href={`/dashboard/dealer/inventory/${vehicle.id}/edit` as Route}>Edit</Link></Button></div></CardContent></Card>)}</div>}</div>;
}
