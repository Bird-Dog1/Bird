...
export default async function DealerDashboardPage() {
  const { user } = await requireRole(["dealer"], "/dashboard/dealer"); const supabase = await createServerSupabaseClient();
  const { data: dealerships, error: dealershipError } = await listAccessibleDealerships(supabase, user.id); const ids = dealerships.map((d) => d.id);
